...
// For older microcontrollers and speed above 100kHz uncomment next line
//#define TWI_FAST_MODE

// 100kHz clock for most of the original ATmega processors
unsigned long const F_TWI = 100000L;         // Hardware I2C clock in Hz

// 100kHz clock for the new 0-series and 1-series ATtiny and ATmega microcontrollers (+ uncomment DXCORE for AVR...Dx in .h)
unsigned long const FREQUENCY = 100000L;     // Hardware I2C clock in Hz

#include "TinyI2CMaster.h"

// Conter for read bytes
int32_t I2Ccount;
...
uint8_t TinyI2CMaster_transfer (uint8_t data) {
...
uint8_t TinyI2CMaster_read (void) {
  ...
  uint8_t data = TinyI2CMaster_transfer(USISR_8bit);

  /* Prepare to generate ACK (or NACK in case of End Of Transmission) */
  if (I2Ccount == 0) USIDR = 0xFF; else USIDR = 0x00;
  TinyI2CMaster_transfer(USISR_1bit);                            // Generate ACK/NACK.
...
bool TinyI2CMaster_write (uint8_t data) {
  ...
  TinyI2CMaster_transfer(USISR_8bit);                            // Send 8 bits on bus.

  /* Clock and verify (N)ACK from slave */
  DDR_USI &= ~(1<<PIN_USI_SDA);                                   // Enable SDA as input.
  if (TinyI2CMaster_transfer(USISR_1bit) & 1<<TWI_NACK_BIT) return false;
...
bool TinyI2CMaster_start (uint8_t address, int32_t readcount) {
  ...
  /* Clock and verify (N)ACK from slave */
  DDR_USI &= ~(1<<PIN_USI_SDA);                                   // Enable SDA as input.
  if (TinyI2CMaster_transfer(USISR_1bit) & 1<<TWI_NACK_BIT) return false; // No ACK

  return true;                                                    // Start successfully completed
}

bool TinyI2CMaster_restart(uint8_t address, int32_t readcount) {
  return TinyI2CMaster_start(address, readcount);
}
...
#elif defined(DXCORE) || defined(TWI0)
...
void TinyI2CMaster_init () {
#if !defined(DXCORE)
  SET_INPUT_PULLUP_TWI_SDA;                                       // Pullups on unless AVR DA/DB
  SET_INPUT_PULLUP_TWI_SCL;
#endif
